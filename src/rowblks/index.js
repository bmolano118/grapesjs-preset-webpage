export default function (editor, config = {}) {
  let rm = editor.RowblkManager;
  let rowblks = config.rowblks;
  let stylePrefix = config.stylePrefix;
  const flexGrid = config.flexGrid;
  const basicStyle = config.addBasicStyle;
  ...
  const toAdd = name => rowblks.indexOf(name) >= 0;
  const attrsRow = attrsToString(rowAttr);
  const attrsCell = attrsToString(colAttr);

  toAdd('column1') && rm.add('column1', {
  ...
  toAdd('column3-7') && rm.add('column3-7', {
